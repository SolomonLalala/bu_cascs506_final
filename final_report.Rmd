...
-   Parse relevant metadata and DNA methylation matrix from GEO database.
-   The metadata and DNA methylation matrix are saved in data_dir, and a list containing the two dataframe is returned.

```{r parse data, eval = FALSE}
# parse data from the GEO database
...
- r is the correlation coefficient between actual age and predicted age. - The model is considered highly accurate when r >= 0.8 on a large independent testing set (n = 62 if r = 0.9) with a broad range of chronological ages ranging from 20 to 100 years. 
```{r view evaluation}
